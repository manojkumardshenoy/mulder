...
type
  TALACDecoder = class(TDecoder)
  private
    function Filter2(const S: String): Boolean;
  protected
  public
...
function TALACDecoder.DecodeFile(const Input: String; const Output: String; const Process: TRunProcess): TProcessResult;
var
  cmd: String;
begin
...
function TALACDecoder.IsFormatSupported(const Container: String; const Flavor: String; const Format: String; const Version: String; const Profile: String): Boolean;
begin
  Result := False;

  if SameText(Container, 'MPEG-4') or SameText(Container, 'ADTS') then
  begin
    if SameText(Format, 'ALAC') then
    begin
      Result := True;
    end;
  end;
...
function TALACDecoder.Filter2(const S: String): Boolean;
begin
  Result := pos('total:', S) = 0;
end;
