...
type
  TShortenDecoder = class(TDecoder)
  private
...
function TShortenDecoder.DecodeFile(const Input: String; const Output: String; const Process: TRunProcess): TProcessResult;
var
  cmd: String;
begin
...
function TShortenDecoder.IsFormatSupported(const Container: String; const Flavor: String; const Format: String; const Version: String; const Profile: String): Boolean;
begin
  Result := False;

  if SameText(Container, 'Shorten') then
  begin
    if SameText(Format, 'Shorten') then
    begin
      Result := True;
    end;
  end;
...
function TShortenDecoder.IsExtensionSupported(const Extension: String): Boolean;
begin
  Result := SameText(Extension, 'shn');
end;

function TShortenDecoder.Filter(const S: String): Boolean;
begin
  Result := True;
end;
