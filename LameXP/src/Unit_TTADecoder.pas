...
function TTTADecoder.DecodeFile(const Input: String; const Output: String; const Process: TRunProcess): TProcessResult;
var
  cmd: String;
begin
...
function TTTADecoder.IsFormatSupported(const Container: String; const Flavor: String; const Format: String; const Version: String; const Profile: String): Boolean;
begin
  Result := False;

  if SameText(Container, 'TTA') then
  begin
    if SameText(Format, 'TTA') then
    begin
      Result := True;
    end;
  end;
...
function TTTADecoder.IsExtensionSupported(const Extension: String): Boolean;
begin
  Result := SameText(Extension, 'tta');
end;

function TTTADecoder.Filter(const S: String): Boolean;
begin
  Result := True;
end;
