...
type
  TWavpackDecoder = class(TDecoder)
  private
  protected
  public
...
function TWavpackDecoder.DecodeFile(const Input: String; const Output: String; const Process: TRunProcess): TProcessResult;
var
  cmd: String;
begin
...
function TWavpackDecoder.IsFormatSupported(const Container: String; const Flavor: String; const Format: String; const Version: String; const Profile: String): Boolean;
begin
  Result := False;

  if SameText(Container, 'WavPack') then
  begin
    if SameText(Format, 'WavPack') then
    begin
      Result := True;
    end;
  end;
...
function TWavpackDecoder.IsExtensionSupported(const Extension: String): Boolean;
begin
  Result := (Extension = 'wv') or (Extension = 'wvc');
end;

function TWavpackDecoder.Filter(const S: String): Boolean;
begin
  Result := pos('% done', S) = 0;
end;
