...
_again:
	...
	if (code == XVID4_RESPONSE_LOAD_MATRIX)
	{
		...
		if (!FileSel_SelectRead(QT_TR_NOOP("Select Xvid matrix file to load"), name, 1023, NULL))
			goto _again;
...
_loop:
	...
	if (code == XVID4_RESPONSE_SAVE_MATRIX)
	{
		...
		if (!FileSel_SelectWrite(QT_TR_NOOP("Select Custom Matrix File to write"), name, 1023, NULL))
			goto _loop;
...
create_dialog1 (void)
{
  ...
  gtk_window_set_title (GTK_WINDOW (dialog1), QT_TR_NOOP("Xvid Configuration"));
  ...
  _0___none = gtk_menu_item_new_with_mnemonic (QT_TR_NOOP("0 - None"));
  gtk_widget_show (_0___none);
  gtk_container_add (GTK_CONTAINER (menu2), _0___none);

  _1___low = gtk_menu_item_new_with_mnemonic (QT_TR_NOOP("1 - Low"));
  gtk_widget_show (_1___low);
  gtk_container_add (GTK_CONTAINER (menu2), _1___low);

  _2___medium = gtk_menu_item_new_with_mnemonic (QT_TR_NOOP("2 - Medium"));
  gtk_widget_show (_2___medium);
  gtk_container_add (GTK_CONTAINER (menu2), _2___medium);

  _3___high = gtk_menu_item_new_with_mnemonic (QT_TR_NOOP("3 - High"));
  ...
  labelIFrameIntervalMax = gtk_label_new (QT_TR_NOOP("Max I-frame interval:"));
