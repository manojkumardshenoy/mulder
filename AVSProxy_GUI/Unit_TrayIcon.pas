////////////////////////////////////////////////////////////////////////////
// ADM Tools - Avidemux Utilities for Win32                               //
...
type
  ...
  public
    ...
    procedure Popup(Sender: TObject); virtual;
  published
...
begin
  ...
  fVisible := Shell_NotifyIcon(NIM_ADD, @fData);
  if not fVisible then raise Exception.Create('Cannot create shell notification icon!')
end;

procedure TTrayIcon.Hide;
begin
  if not fVisible then Exit;
  fVisible := not Shell_NotifyIcon(Nim_Delete, @fData);
end;

procedure TTrayIcon.Popup(Sender: TObject);
begin
  DoMenu;
end;
...
begin
  fIcon.Free;
  if not (csDesigning in ComponentState) then
    if fVisible then Shell_NotifyIcon(Nim_Delete, @fData);
  inherited Destroy;
end;
...
begin
  case Msg.Msg of
    ...
    if (Msg.Msg = WM_TASKBAR_RESTART) and fVisible then
      Shell_NotifyIcon(NIM_ADD, @fData); // if EXPLORER.EXE was restarted, we have to re-create the TrayIcon! +++ Fixed by MuldeR
  end;
...
var
  Pt: TPoint;
begin
  if (fPopupMenu <> nil) then // and not IsWindowVisible(Application.Handle) +++ Edited by MuldeR
  begin
    GetCursorPos(Pt);
